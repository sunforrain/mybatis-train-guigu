 ...
 * 								STATEMENT：可以禁用一级缓存；
 *
 *视频59 缓存_MyBatis整合ehcache&总结
 *第三方缓存整合(用的缓存是ehcache)：
 *		1）、导入第三方缓存包即可；
...
public class MyBatisTest {
	...
	public void testSecondLevelCache02() throws IOException{
		...
		try{
			// 视频55 缓存_二级缓存使用&细节
			// mapper.xml没有配置<cache></cache>,则二级缓存不会生效
	...
	/**
	 * mybatis的二级缓存底层就是个map,并不专业,缓存的问题还是交给其他专业的三方缓存较好
	 * 二级缓存：（全局缓存）：基于namespace级别的缓存：一个namespace对应一个二级缓存：
	 * 		工作机制：
	 * 		1、一个会话，查询一条数据，这个数据就会被放在当前会话的一级缓存中；
	 * 		2、如果会话关闭；一级缓存中的数据会被保存到二级缓存中；新的会话查询信息，就可以参照二级缓存中的内容；
	 ...
	 * 视频56 缓存_缓存有关的设置以及属性
	 * 和缓存有关的设置/属性：
	 * 			1）、cacheEnabled=true(settings中设置)：false：关闭缓存（二级缓存关闭）(一级缓存一直可用的)
	 * 			2）、每个select标签都有useCache="true"：
	 * 					false：不使用缓存（一级缓存依然使用，二级缓存不使用）
	 * 			3）、【每个增删改标签的：flushCache="true"：（一级二级都会清除）】
	 * 					增删改执行完成后就会清楚缓存；
	 ...
	 * 						如果flushCache=true;每次查询之后都会清空缓存；缓存是没有被使用的；
	 * 			4）、sqlSession.clearCache();只是清楚当前session的一级缓存；
	 * 			5）、localCacheScope(settings中设置)：本地缓存作用域：（一级缓存SESSION）；当前会话的所有数据保存在会话缓存中；
	 * 								STATEMENT：可以禁用一级缓存；
	 * @throws IOException
	 */
	@Test
